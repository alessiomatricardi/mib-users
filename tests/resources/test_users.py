...
class TestUsers(ViewTest):
    ...
    def test_04_pictures(self):
        ...
        json_pfp_updating_success = {'requester_id': test_id['id'], 'image': img_base64}
        ...
        rv = self.client.put('/profile/picture', json = json_pfp_updating_success)
        self.assertEqual(rv.status_code, 200)
        ...
        # get json variables
        json_get_correct_id = {'requester_id': test_id['id']}
        json_get_wrong_id = {'requester_id': 200}

        # retrieving a profile picture with a wrong current user id
        rv = self.client.get('/users/%s/picture' % (str(test_id['id'])), json = json_get_wrong_id)
        self.assertEqual(rv.status_code, 404)

        # retrieving a profile picture with a wrong target user id
        rv = self.client.get('/users/200/picture', json = json_get_correct_id)
        self.assertEqual(rv.status_code, 404)

        # retrieving a profile picture with correct id
        rv = self.client.get('/users/%s/picture' % (str(test_id['id'])), json = json_get_correct_id)
        self.assertEqual(rv.status_code, 200)
        ...
        rv = self.client.get('/users/2/picture', json = json_get_correct_id)
        self.assertEqual(rv.status_code, 200)
    ...
    def test_06_get_arbitrary_user_info(self):
        ...
        rv = self.client.get('/users/200', json = json_get_correct_id)
        self.assertEqual(rv.status_code, 404)

        # retrieving an existing target user from a non existing current user
        rv = self.client.get('/users/%s' % (str(test_id['id'])), json = json_get_wrong_id)
        self.assertEqual(rv.status_code, 404)
    ...
    def test_09_unregister(self):
        ...
        json_user_unregistration_id_failure= { 'requester_id' : 200  , 'password': 'abcd1234'} 
        json_user_unregistration_password_failure= { 'requester_id' : test_id['id']  , 'password': 'notapassword'} 
        json_user_unregistration_success= { 'requester_id' : test_id['id']  , 'password': 'abcd1234'} 

        # trying to unregister a non existing user
        rv = self.client.put('/unregister', json = json_user_unregistration_id_failure)
        self.assertEqual(rv.status_code, 404)

        # trying to unregister an existing user using the wrong password
        rv = self.client.put('/unregister', json = json_user_unregistration_password_failure)
        self.assertEqual(rv.status_code, 401)

        # unregistering an existing user using the correct password
        rv = self.client.put('/unregister', json = json_user_unregistration_success)
        self.assertEqual(rv.status_code, 200)
